...
class listtab():
    def __init__(self, mycursor, tablename):
        ...
        #self.savefilevar = savefile.savingdata(self.pww,self.cursor,self.tablename)
...
##        columnname = self.getcolumnnames(buttonnumber,tablename)
        ...
        self.pww.destroy()
        changesdone = 1
        listtab(self.cursor,self.tablename)
    ...
    def editlist(self,row,column,buttonnumber,boxname):
        ...
        but = Button(self.pww, text = 'Modify Value', width = 15, command = lambda : self.savechanges(labels[0],but,boxname,buttonnumber,row,listnames))
        but.grid(column = 0, row = 210)
...
class databasewin():
    def __init__(self,mycursor,conn):
        self.conn = conn
        self.cursor = mycursor
...
if __name__ == "__main__":
    conn = mys.connect(host = 'localhost', user = 'root', passwd = "password")
    mycursor = conn.cursor()
    databasewin(mycursor,conn)
